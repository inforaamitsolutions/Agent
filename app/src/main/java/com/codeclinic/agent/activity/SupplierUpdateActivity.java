...
public class SupplierUpdateActivity extends AppCompatActivity {
    ...
    private void getSurveyForm() {
        disposable.add(localDatabase.getDAO().getSupplierSurveyFormList()
            ...
            disposable.add(RestClass.getClient().BUSINESS_DATA_SUBMIT_FORM_MODEL_SINGLE_CALL(sessionManager.getTokenDetails().get(SessionManager.AccessToken)
                    ...
                    .subscribeWith(new DisposableSingleObserver<BusinessDataSubmitModel>() {
                        ...
                        public void onError(@NonNull Throwable e) {
                            binding.loadingView.loader.setVisibility(View.GONE);
                            Toast.makeText(SupplierUpdateActivity.this, "Server Error" + e.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    }));
        } else {
            binding.loadingView.loader.setVisibility(View.GONE);
            saveSupplierFormToLocal(jsonObject.toString());
        }
    }

    private void saveSupplierFormToLocal(String request) {
        SupplierFinalFormEntity formEntity = new SupplierFinalFormEntity();
        formEntity.setMainId(0);
        formEntity.setRequest(request);
        disposable.add(Completable.fromAction(() -> localDatabase.getDAO()
                .saveSupplierFinalForm(formEntity))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    private void updatePage() {
        ...
        if (question.getFieldType().equals("select_one") || question.getFieldType().equals("select_multiple")) {
            ...
            spAdapter = new ArrayAdapter(SupplierUpdateActivity.this, R.layout.spinner_item_view, question.getOptions());
